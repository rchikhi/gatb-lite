...
struct file_descriptor : public bound_cursor
{
    ...
    static file_descriptor make_shm(const char* name = "tmp", int oflag = O_RDWR | O_CREAT, mode_t mode = 0700, bool unlink = true)
    {
        const unsigned fd = sys::check_ret(::shm_open(name, oflag, mode), "shm_open");
        if (unlink) {
            sys::check_ret(::shm_unlink(name), "shm_unlink");
        }
    ...
    mmap_range<T> mmap(off_t offset = 0, size_t len = 0, int prot = PROT_READ, int flags = MAP_PRIVATE, std::remove_const_t<T>* addr = nullptr)
    {
        if (offset == 0 && len == 0) {
            len = this->size() / sizeof(T);
        }
        return mmap_range<T>(_fd, len, prot, flags, addr, offset);
    }

    template<typename Buffer>
    size_t pread(Buffer buf, off_t offset) const
    {
        assume(buf.size() > 0);
    ...
    size_t readahead(off64_t offset, size_t count) const { return sys::check_ret(::readahead(_fd, offset, count), "readahead"); }

    void close()
    {
        if (_fd >= 0) {
            sys::check_ret(::close(_fd), "close");
        }
        _fd = -1;
    }
