...
template<typename T>
class mmap_range : public unique_range<T, details::munmapper<T>>
{
  ...
  public:
    using base::operator=;
    mmap_range(int fd, size_t len, int prot = PROT_READ, int flags = MAP_PRIVATE, std::remove_const_t<T>* addr = nullptr, off_t offset = 0)
      : base()
    {
    ...
    void advise_sequential(size_t len = 0, size_t from = 0) { this->advise(MADV_SEQUENTIAL, len, from, "madvise(MADV_SEQUENTIAL, %2$lu, %3$d)"); }

    void advise_hugepage(size_t len = 0, size_t from = 0) { this->advise(MADV_HUGEPAGE, len, from, "madvise(MADV_HUGE_PAGE, %2$lu, %3$d)"); }

  protected:
    void advise(int advice, size_t len = 0, size_t from = 0, const char error_msg[] = "madvise(%p, %lu, %d)")
    {
        len = len > 0 ? len : this->size();
        assume(from + len <= this->size(), "advise past the end (offset+len=%lu, size()=%lu)", from + len, this->size());
