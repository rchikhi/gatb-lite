...
public:
    using base::operator=;
    mmap_range(int fd, size_t len, int prot = PROT_READ, int flags = MAP_PRIVATE, std::remove_const_t<T>* addr = nullptr, off_t offset = 0): base() {
    ...
    bool advise_sequential(size_t len=0, size_t from=0) {
        ...
        return ::madvise( unmapper::unconst_void(this->begin() + sizeof(T) * from), sizeof(T) * len, MADV_SEQUENTIAL) >= 0;
    }
