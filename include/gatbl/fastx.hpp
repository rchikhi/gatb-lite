...
struct sequence_iterator : private utils::precondition_check<>
{
...
template<typename Record>
struct std::iterator_traits<gatbl::sequence_iterator<Record>>
{
...
template<typename Record, typename Range>
inline auto
seq_record_subrange(const Range& char_range) -> decltype(seq_record_subrange<Record>(char_range.begin(), char_range.end()))
{
...
class fasta_record
{
    ...
    static_assert(std::is_same_v<typename char_iterator_trais::iterator_category, std::random_access_iterator_tag>, "Random acces iterator required");
...
template<typename CharIt = const char*>
class fastq_record : public fasta_record<CharIt, '@'>
{
  ...
  public:
    ...
    const substring_t quality() const { return { _quality_header_end + 1, _quality_header_end + 1 + this->sequence().size() }; }
