...
/// Construct a range wrapper giver an isomorphism functor (with one overload of operator() for each side of the bijection)
template<typename Rep, typename Iso>
class range_view : public utils::empty_base<Rep, Iso>
{
    ...
    template<typename T>
    class wreference : protected utils::empty_base<T, Iso>
    {
      ...
      public:
        ...
        template<typename V>
        auto operator=(V&& v) -> decltype(concepts::type_require<wreference&>(this->value() = this->tag()(std::forward<V>(v))))
        {
    ...
    template<typename T>
    class witerator : protected utils::empty_base<T, Iso>
    {
