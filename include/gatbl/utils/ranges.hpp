...
auto
ZippableRanges(X& x, Y& y, Zipper&& binop = Zipper{}) -> decltype(value_require(Ranges(x, y), binop(*begin(x), *begin(y))));
...
template<typename Range>
using reverse_iterator_t = decltype(rbegin(std::declval<Range&>()));
template<typename Range>
using reverse_sentinel_t = decltype(rend(std::declval<Range&>()));
template<typename Range>
using reference_t = decltype(*begin(std::declval<Range&>()));
template<typename Range>
using value_t = std::remove_reference_t<reference_t<Range>>;
...
// inline constexpr bool operator<=(const X& x, const Y& y) noexcept { return lexicographical_compare(x, y, std::less_equal{}); }

// template<typename X,typename Y, typename=decltype(concepts::ZippableRanges<X, Y, std::greater<>>)>
// inline constexpr bool operator>(const X& x, const Y& y) noexcept { return lexicographical_compare(x, y, std::greater{}); }

// template<typename X,typename Y, typename=decltype(concepts::ZippableRanges<X, Y, std::greater_equal<>>)>
// inline constexpr bool operator>=(const X& x, const Y& y) noexcept { return lexicographical_compare(x, y, std::greater_equal{}); }
